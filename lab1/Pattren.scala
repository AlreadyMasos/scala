...
object PatternMatching {
  ...
  def testIntToString(value: Int): String = take(value.toString)
   ...
   *     "moritz" или "Moritz"
   */
  def MaxMortiz(value: String): Boolean = {
    value match{
      case "max" => true
      case "Max" => true
      case "moritz" => true
  ...
  def testIsMaxAndMoritz(value: String): Boolean = MaxMortiz("das")

  // c) Напишите функцию проверки является ли `value` четным
  def Chet(value: Int): Boolean = value % 2 == 0


  // примените функции из пункта (c) здесь, не изменяя сигнатуру
  def testIsEven(value: Int): Boolean = Chet(5)
   ...
   *    Выиграет ли игрок `a`?
   */
  def RockPaperScissors(a: Hand, b: Hand): Unit ={
    a match {
      case "rock" => b == "paper"
  ...
  def testExtractMammalWeight(animal: Animal): Int = tapWeight(animal)
